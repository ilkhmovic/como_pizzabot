...
def check_click_request(request_data: dict, action: str) -> bool:
    ...
    try:
        ...
        try:
            f = float(raw_amount)
            amount_candidates.append("{:.2f}".format(f))
...
async def main():
    ...
    # YANGI: Database initialization try-catch ichida
    try:
        init_db()
        logger.info("✅ Ma'lumotlar bazasi sozlandi")
    except Exception as e:
        logger.error(f"❌ Ma'lumotlar bazasi sozlash xatosi: {e}")
        # Xatoga qaramay davom etish
